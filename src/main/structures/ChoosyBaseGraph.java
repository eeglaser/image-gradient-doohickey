...
public class ChoosyBaseGraph<NodeType, EdgeType extends Number> {
   ...
   * 
   * @param map the map the graph uses to map a data object to the node object it is stored in
   */
  public ChoosyBaseGraph(MapADT<NodeType, Node> map) {
    this.nodes = map;
  }
  ...
  /**
   * Sets the disabled property of a node given its key. True is disabled, false is enabled.
   * 
   * @param n    The NodeType for the node to set the field of
   * @param bool Whether this node should be disabled (true) or enabled (false)
   */
  public void setIsDisabled(NodeType n, boolean bool) {
    nodes.get(n).isDisabled = bool;
  }

  /**
   * Checks if a node is disabled.
   * 
   * @param n the node to check the disabled property of
   * @return true if the node is disabled, false otherwise
   */
  public boolean getIsDisabled(NodeType n) {
    return nodes.get(n).isDisabled;
  }
}
